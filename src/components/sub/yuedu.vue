<template>
  <div class="yuedu">
    ...
    <mt-button type="danger" size="small" @click="read">立即阅读</mt-button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    ifFlag() {
      this.flag = false;
      var carbooks = JSON.parse(
        window.localStorage.getItem("SHEFLBOOK") || "{}"
      );
    ...
    read() {
      this.$router.push({ name: "read", params: { name: this.title } }); //从book.vue 传来的id  跳转时把id带到详情
    },
  },
};
</script>
